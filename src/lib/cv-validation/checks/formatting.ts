...
export function checkFormatting(cv: CvInput, locale: Locale): SectionScore {
  ...
  if (words >= 450 && words <= 1200) {
    ...
    earned += 3
    if (words < 450) {
      issues.push({ text: `CV has ${words} words — aim for 450-1200 for optimal ATS performance`, priority: 'recommended', section: 'general' })
    } else {
      issues.push({ text: `CV has ${words} words — consider trimming to under 1200`, priority: 'recommended', section: 'general' })
    }
  } else if (words > 0) {
    earned += 1
    issues.push({ text: `CV has ${words} words — far from the 450-1200 ideal range`, priority: 'critical', section: 'general' })
  } else {
    issues.push({ text: 'CV appears to be empty', priority: 'critical', section: 'general' })
  }
  ...
  if (emptyBullets.length === 0) {
    ...
    issues.push({ text: `${emptyBullets.length} empty bullet point(s) found — remove or fill them`, priority: 'recommended', section: 'general' })
  }
  ...
  if (dates.length > 0) {
    ...
    if (safe.length === dates.length) {
      ...
      issues.push({ text: `${unsafe} date(s) may not be ATS-friendly — use "Month YYYY" or "MM/YYYY" format`, priority: 'recommended', section: 'general' })
    }
  } else {
  ...
  if (!pattern.test(allText)) {
    ...
    issues.push({ text: 'First-person pronouns detected — remove "I", "my", etc. for a professional tone', priority: 'recommended', section: 'general' })
  }
